...
export default function KnowledgeListModal({ isOpen, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/40 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-3xl max-h-[90vh] rounded-3xl border border-stone-200 shadow-2xl overflow-hidden flex flex-col">
        <header className="px-6 py-4 border-b border-stone-200 flex items-center justify-between bg-linear-to-r from-indigo-500/10 to-emerald-500/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-black text-lg tracking-tight text-stone-800">
                Cerebro de Kira
              </h2>
          ...
          <button
            onClick={onClose}
            className="text-stone-500 hover:text-stone-800 hover:bg-stone-100 p-2 rounded-full transition-colors"
            aria-label="Cerrar"
          >
        ...
        <div className="px-6 py-3 border-b border-stone-200 flex gap-2 items-center bg-milokira-crema/40">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-stone-50 border border-stone-200 rounded-lg text-sm text-stone-800 placeholder-stone-400 outline-none focus:border-emerald-500 transition-colors"
            />
          </div>
          <button
            onClick={() => {
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {loading ? (
            <div className="text-center py-10 text-stone-500 text-sm">Cargando...</div>
          ) : filtered.length === 0 ? (
            ...
            <ul className="space-y-2">
              {filtered.map((item) => (
                <li
                  key={item.idFirebase}
                  className="bg-white border border-stone-200 rounded-xl p-3 hover:border-stone-300 shadow-sm transition-colors"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide bg-milokira-lila/30 text-stone-700 border border-milokira-lila">
                          {item.categoria}
                        </span>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {item.keywords.slice(0, 6).map((k) => (
                          <span
                            key={k}
                            className="text-[10px] bg-white text-stone-500 px-1.5 py-0.5 rounded border border-stone-200"
                          >
                            {k}
                          </span>
                        ))}
                    ...
                    <div className="flex gap-1 shrink-0">
                      <button
                        ...
                        className="p-2 bg-milokira-lila/20 text-stone-700 border border-milokira-lila rounded-lg hover:bg-milokira-lila/40 transition-colors"
                        title="Editar"
                      >
                        <Edit3 size={14} />
                      </button>
                      <button
                        ...
                        className="p-2 bg-rose-50 text-rose-600 border border-rose-200 rounded-lg hover:bg-rose-100 transition-colors"
                        title="Eliminar"
                      >
                        <Trash2 size={14} />
                      </button>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-stone-900/40 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-xl rounded-3xl border border-stone-200 shadow-2xl overflow-hidden">
        <header className="px-6 py-4 border-b border-stone-200 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="text-stone-500 hover:text-stone-800 hover:bg-stone-100 p-1.5 rounded-full"
          >
            <X size={18} />
          </button>
        </header>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm text-stone-800 placeholder-stone-400 outline-none focus:border-emerald-500 transition-colors"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm text-stone-800 outline-none focus:border-emerald-500 transition-colors"
            >
              {CATEGORIAS.map((c) => (
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm text-stone-800 placeholder-stone-400 outline-none focus:border-emerald-500 transition-colors resize-none"
            />
          </div>
