...
import { Search, X, Truck, Droplet, Info, MapPin, ChevronDown } from "lucide-react";
...
export default function Home() {
  ...
  // Reset visibleCount cuando cambia filtro/búsqueda — patrón derived state.
  // Mantenemos el último filtro/búsqueda como referencia y reseteamos en render.
  const [lastFilter, setLastFilter] = useState({
    cat: categoriaActiva,
    q: busqueda,
  });
  if (lastFilter.cat !== categoriaActiva || lastFilter.q !== busqueda) {
    setVisibleCount(PAGE_SIZE);
    setLastFilter({ cat: categoriaActiva, q: busqueda });
  }
  useEffect(() => {
  ...
  return (
    <CartProvider>
      ...
      >
        ...
        {/* FAB WhatsApp para clientes */}
        {!isAdmin && (
          <a
            href="https://wa.me/56994955949?text=Hola%2C%20vi%20el%20cat%C3%A1logo%20de%20Milokira%20y%20quiero%20consultar%20por%20una%20planta%20%F0%9F%8C%BF"
            target="_blank"
            rel="noopener noreferrer"
            aria-label="Consultar por WhatsApp"
            className="group fixed bottom-5 right-5 sm:bottom-6 sm:right-6 z-40 inline-flex items-center gap-2 bg-[#25D366] hover:bg-[#1DA851] text-white pl-4 pr-5 py-3 rounded-full shadow-lg shadow-emerald-900/30 transition-all duration-300 hover:scale-105 active:scale-95"
          >
            <svg
              viewBox="0 0 24 24"
              fill="currentColor"
            ...
            <span className="text-sm font-bold sm:hidden">Comprar</span>
          </a>
        )}
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 pt-4 sm:pt-6 pb-3 flex items-center justify-between gap-2">
          ...
          <a
            href="#ubicacion"
            className="hidden xs:inline-flex items-center gap-1 sm:gap-1.5 bg-white/80 backdrop-blur-sm border border-milokira-verde/30 text-milokira-verde rounded-full px-2.5 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-xs font-bold shadow-sm hover:bg-white hover:scale-105 transition-all"
            title="Ver ubicación en el mapa"
          >
            <MapPin size={12} strokeWidth={2.5} className="shrink-0" />
            <span>
              Bicentenario, <span className="hidden sm:inline">Talca</span>
            </span>
          </a>
          ...
          <div className="flex items-center gap-1 sm:gap-3">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 sm:p-2.5 rounded-full hover:bg-milokira-lila/40 hover:scale-110 transition-all duration-300"
              title="WhatsApp"
            >
              <svg
                viewBox="0 0 24 24"
                fill="currentColor"
                className="w-7 h-7 sm:w-8 sm:h-8 text-milokira-verde"
              >
        ...
        <section className="relative max-w-7xl mx-auto mt-2 sm:mt-3 mb-8 sm:mb-10 px-2 sm:px-4">
          <div className="relative w-full sm:aspect-[5/2] sm:max-h-[540px] overflow-hidden sm:rounded-[40px] sm:shadow-xl sm:shadow-milokira-verde/10 sm:border sm:border-white/60">
            ...
            <span
              aria-hidden="true"
              className="pointer-events-none absolute top-4 left-3 sm:top-8 sm:left-12 text-milokira-verde/40 animate-float-slow"
            ...
            <span
              aria-hidden="true"
              className="pointer-events-none absolute top-6 right-4 sm:top-12 sm:right-16 text-milokira-lila/60 animate-float-slow"
              style={{ animationDelay: "1.2s" }}
            >
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 22C7.03 22 3 17.97 3 13c0-3.66 2.13-6.83 5.27-8.21A9.96 9.96 0 0112 4c.97 0 1.92.14 2.82.41C18.74 5.66 21 9.05 21 13c0 4.97-4.03 9-9 9zm-1-2c4.42 0 8-3.58 8-8 0-3.04-1.7-5.7-4.2-7.03C12.13 4.36 9.32 5.96 7.4 8.4 5.5 10.84 4.5 13.83 5 17c.45 1.83 2.93 3 6 3z" />
              </svg>
            </span>
            <span
              aria-hidden="true"
              className="pointer-events-none absolute bottom-6 left-5 sm:bottom-12 sm:left-20 text-milokira-verde/30 animate-float-slow"
            ...
            <span
              aria-hidden="true"
              className="pointer-events-none absolute bottom-8 right-6 sm:bottom-16 sm:right-24 text-milokira-lila/40 animate-float-slow"
            ...
            <div className="sm:absolute sm:inset-0 flex items-center justify-center py-6 sm:py-0">
              <div className="text-center px-6 relative z-10">
                <h1
                  className="text-5xl sm:text-5xl md:text-6xl font-black tracking-tight text-stone-800 leading-[1.05] drop-shadow-sm"
                ...
                <p className="mt-3 sm:mt-3 text-stone-600 text-base sm:text-sm md:text-base font-medium italic max-w-xs sm:max-w-md mx-auto leading-snug">
                  Con cariño desde Talca para toda tu casa.
                  <br className="hidden sm:block" />{" "}
                  Inspirado en nuestros tres peludos: Milo, Loki y Kira 🐾
                </p>

                {/* Chip de ubicación + envíos bajo el hero */}
                <a
                  href="#ubicacion"
                  className="mt-4 sm:mt-5 inline-flex flex-wrap items-center justify-center gap-2 bg-white/80 backdrop-blur-sm border border-milokira-verde/30 rounded-full px-3 sm:px-4 py-1.5 sm:py-2 shadow-sm hover:bg-white hover:scale-105 transition-all"
                  title="Ver ubicación en el mapa"
                >
                  <span className="inline-flex items-center gap-1 text-milokira-verde text-xs sm:text-sm font-bold">
                    <MapPin size={14} strokeWidth={2.5} />
                    Bicentenario, Talca
                  </span>
                  <span className="text-stone-300">·</span>
                  <span className="inline-flex items-center gap-1 text-stone-600 text-xs sm:text-sm font-medium">
                    <Truck size={14} strokeWidth={2.5} className="text-milokira-verde" />
                    Envíos a todo Chile
                  </span>
                </a>
              </div>
        ...
        <div className="mb-8 sm:mb-10 max-w-6xl mx-auto flex sm:flex-wrap sm:justify-center gap-2.5 sm:gap-3 px-4 sm:px-8 py-2 overflow-x-auto scrollbar-hide snap-x snap-mandatory">
        ...
        <div className="max-w-xl mx-auto mb-8 sm:mb-10 px-4 sm:px-8">
      ...
      */}
        ...
        <div
          id="catalogo"
          className={`scroll-mt-24 grid ${categoriaActiva === "IMPLEMENTOS" ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" : "grid-cols-2 lg:grid-cols-3"} gap-3 sm:gap-8 max-w-6xl mx-auto`}
        >
          {isLoadingData &&
            Array(6)
              .fill(0)
          ...
          {!isLoadingData &&
            ...
            plantasFiltradas.slice(0, visibleCount).map((planta) => (
              <PlantCard
                key={planta.id}
                planta={planta}
                isAdmin={isAdmin}
        ...
        {/* Botón "Ver más" */}
        {!isLoadingData && plantasFiltradas.length > visibleCount && (
          <div className="max-w-3xl mx-auto mt-8 sm:mt-10 px-4 flex flex-col items-center gap-2">
            <button
              type="button"
              onClick={() =>
                setVisibleCount((c) =>
                  Math.min(c + PAGE_SIZE, plantasFiltradas.length),
                )
              }
              className="inline-flex items-center gap-2 bg-white border-2 border-milokira-verde/40 text-milokira-verde hover:bg-milokira-verde hover:text-white font-bold px-6 py-3 rounded-full shadow-sm hover:shadow-lg hover:scale-105 active:scale-95 transition-all duration-300 text-sm sm:text-base"
            >
              Ver más plantitas
              <ChevronDown size={18} strokeWidth={2.5} />
            </button>
            <span className="text-xs text-stone-500 font-medium">
              Mostrando {visibleCount} de {plantasFiltradas.length}
            </span>
          </div>
        )}

        {/* Guía de iconos en mobile, al final del catálogo */}
        <div className="max-w-3xl mx-auto mt-10 mb-4 px-4 sm:hidden">
          <button
            type="button"
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-8">
              ...
              <div className="text-left space-y-3 text-sm text-gray-600 font-medium flex-1 w-full">
                ...
                <p className="flex items-start gap-2.5">
                  ...
                  <span className="leading-relaxed">
                    Delivery{" "}
                    <strong className="text-milokira-verde">GRATIS</strong> en
                    Bicentenario sobre $10.000 (valor adicional a otras zonas).
                  </span>
                </p>
                ...
                <div className="flex items-center justify-center md:justify-start gap-2 pt-3">
                  ...
                  >
                    <svg
                      viewBox="0 0 24 24"
                      fill="currentColor"
                      className="w-4 h-4 text-milokira-verde"
                    >
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                    </svg>
                  </a>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="relative group w-full md:w-64 h-48 rounded-2xl overflow-hidden border border-milokira-verde/20 shadow-sm shrink-0 scroll-mt-24"
              >
                <iframe
